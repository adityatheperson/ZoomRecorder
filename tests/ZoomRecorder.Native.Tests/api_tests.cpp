...
int main() {
  ...
  if (!run_video_frame_normalizer_tests()) return EXIT_FAILURE;
  if (!run_capture_size_state_tests()) return EXIT_FAILURE;
  if (!run_pcm_wav_converter_tests()) return EXIT_FAILURE;
  zr_handle handle{};
