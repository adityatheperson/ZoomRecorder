...
bool run_mp4_writer_tests() {
  ...
  if (!opened || !device_created || !texture_created || !video_written)
    std::fprintf(stderr, "mp4 gpu path: open=%d device=%d texture=%d write=%d\n", opened, device_created, texture_created, video_written);
  writer.finalize();
  DeleteFileW(final_path.c_str());
  DeleteFileW((final_path + L".partial").c_str());
  DeleteFileW((final_path + L".partial.mp4").c_str());
  return opened && video_written;
}
