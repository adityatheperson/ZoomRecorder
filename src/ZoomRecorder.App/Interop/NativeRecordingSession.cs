...
internal sealed class NativeRecordingSession(NativeSession session) : IRecordingSession
{
    ...
    public Task StartAsync(RecordingTarget value, CancellationToken cancellationToken) { cancellationToken.ThrowIfCancellationRequested(); session.StartRecording(value.Path); target = value; startedAt = DateTimeOffset.UtcNow; return Task.CompletedTask; }
