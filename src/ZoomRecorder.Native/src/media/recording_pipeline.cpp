...
class RecordingPipelineImpl {
 public:
  ...
  bool start(const std::wstring& output) {
    ...
    state_changed_.wait_for(lock, std::chrono::seconds(8), [this] { return readiness_.can_enter_before_video() || readiness_.has_failed(); });
    if (!readiness_.can_enter_before_video()) { lock.unlock(); stop_and_finalize(); return fail("Recording audio sources did not become ready"); }
    return true;
  }
